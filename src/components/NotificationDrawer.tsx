...
export default function NotificationDrawer() {
    const { isOpen, setIsOpen, notifications, setNotifications } = useNotificationStore();
    ...
    return (
        <Drawer anchor="right" open={isOpen} onClose={() => setIsOpen(false)}>
            <DrawerList notifications={notifications} activeTab={activeTab} setActiveTab={setActiveTab} markAllAsRead={markAllAsRead} />
        </Drawer>
    )
}

function DrawerList({ notifications, activeTab, setActiveTab, markAllAsRead }: DrawerListProps) {
    return (
        <div className="w-[330px] sm:w-[500px] lg:w-[700px] h-full bg-secondary py-5 px-5 sm:px-11 rounded-l-3xl overflow-y-scroll">
            <div className="flex items-center justify-between">
                ...
                <Dropdown>
                    ...
                    <DropdownMenu className="bg-white rounded-md min-w-[250px] text-sm py-2 px-1">
                        <DropdownItem className="hover:bg-gray-200 rounded-md py-3 px-2" onClick={markAllAsRead}>Mark all as read</DropdownItem>
                    </DropdownMenu>
                </Dropdown>
            </div>
            <div className="flex border-b border-white/10 mt-1 justify-between">
                <div className="flex relative w-full">
                    {['All', 'Unread'].map((tab) => (
                        <button
                            key={tab}
                            className={clsx('py-2 px-4 text-white transition-colors duration-300 w-1/2 text-sm sm:text-base',
                                { 'text-tertiary': activeTab === tab }
                            )}
                            onClick={() => setActiveTab(tab)}
                        >
                            <span className={clsx({ 'text-tertiary': activeTab === tab })}>{tab}</span>
                        </button>
                    ))}
                    <div
            ...
            <div className="mt-5">
                {notifications && notifications.filter(notification => activeTab === 'All' || !notification.read).map((notification) => (
                    <div key={notification.notification_id} className="bg-primary/20 w-full p-3 mb-2">
                        <div className="flex items-start gap-3">
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="flex items-center gap-1">
                                    <svg width="14px" height="14px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M12 7V12L14.5 10.5M21 12C21 16.9706 16.9706 21 12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12Z" stroke="#C3D1C1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path> </g></svg>
