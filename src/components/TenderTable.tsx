...
const TenderTable = ({ tenders }: { tenders?: ITender[] }) => {
  const { archiveTender, unarchiveTender, recordTender, unrecordTender, user } =
    useUserStore((state) => ({
  ...
  const isRecorded = (tender: ITender) =>
    user?.recordedTenders.includes(tender);
  const isArchived = (tender: ITender) =>
    user?.archivedTenders.includes(tender);
  ...
  return (
    <table className="w-full overflow-scroll">
      ...
      <tbody>
            ...
            <tr key={index}>
              ...
              <td>
                <svg
                  ...
                  height="25px"
                  viewBox="0 0 24 24"
                  fill={isArchived(tender) ? "#777777" : "none"}
        ...
        {pathname === "/archive" &&
          user?.archivedTenders.map((tender, index) => (
            <tr key={index}>
              <td>{index + 1}</td>
              {/* <td className="flex items-center gap-2"><DataWithPopOver value={tender.nama_tender} /></td> */}
              <td className="whitespace-normal text-left">
              ...
              <td className="flex items-center gap-1">
                ...
                <svg
                  onClick={() => handleRecord(tender)}
                  className="cursor-pointer"
                  width="25px"
                  height="25px"
                  viewBox="0 0 24 24"
                  fill={isRecorded(tender) ? "#3CDB7F" : "none"}
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <circle
                    cx="12"
                    cy="12"
                    r="10"
                    stroke={isRecorded(tender) ? "#3CDB7F" : "#777"}
                    strokeWidth="1.5"
                  ></circle>
                  <path
                    d="M8.5 12.5L10.5 14.5L15.5 9.5"
                    stroke={isRecorded(tender) ? "#FFFFFF" : "#777"}
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
        ...
        {pathname === "/record" &&
          user?.recordedTenders.map((tender, index) => (
            <tr key={index}>
              <td>{index + 1}</td>
              {/* <td className="flex items-center gap-2"><DataWithPopOver value={tender.nama_tender} /></td> */}
              <td className="whitespace-normal text-left">
  ...
  return (
    <div className="flex justify-between w-full gap-2">
      ...
      <Popover placement="bottom" showArrow>
        ...
        <PopoverContent>
          <p className="py-2 px-3 bg-black/80 text-white text-sm">{value}</p>
        </PopoverContent>
      </Popover>
    </div>
  );
